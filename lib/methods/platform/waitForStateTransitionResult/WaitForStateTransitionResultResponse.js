    ...
    if (proto.getProof()) {
      proof = new Proof({
        ...
        storeTreeProofs: new StoreTreeProofs({
          dataContractsProof:
            Buffer.from(proto.getProof().getStoreTreeProofs().getDataContractsProof()),
          publicKeyHashesToIdentityIdsProof:
            Buffer.from(
              proto.getProof().getStoreTreeProofs().getPublicKeyHashesToIdentityIdsProof(),
            ),
          identitiesProof:
            Buffer.from(proto.getProof().getStoreTreeProofs().getIdentitiesProof()),
          documentsProof:
            Buffer.from(proto.getProof().getStoreTreeProofs().getDocumentsProof()),
        }),
