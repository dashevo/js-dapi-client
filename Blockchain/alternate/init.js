...
exports.init = function () {
  ...
  return async function (query, update) {
    return new Promise((async (resolve, reject) => {
      ...
      self.Blockchain.chain = await new Blkchain({ genesisHeader });
      const chain = self.Blockchain.chain;
      ...
      // Fetching last block
      const lastTip = await self.Explorer.API.getLastBlock();
      ...
      socket.on('connect', () => {
        ...
        socket.on('block', async (_block) => {
          ...
          const block = await self.Explorer.API.getBlock(blockHash);
