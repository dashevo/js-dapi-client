const MaxRetriesReachedError = require('../../methods/errors/MaxRetriesReachedError');
const NoAvailableAddressesForRetryError = require('../../methods/errors/NoAvailableAddressesForRetryError');
  ...
  async request(method, params = {}, options = {}) {
    ...
    try {
      ...
      if (options.retries === 0) {
        throw new MaxRetriesReachedError(error.code, error.metadata, undefined, address);
      }

      const hasAddresses = await dapiAddressProvider.hasLiveAddresses();
      if (!hasAddresses) {
        throw new NoAvailableAddressesForRetryError(error.code, error.metadata, undefined, address);
      }
