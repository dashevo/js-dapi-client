...
function createAddressProviderFromOptions(options) {
  ...
  if (options.seeds) {
    ...
    const addressProvider = new ListAddressProvider(
    ...
    const dmlProvider = new SMLProvider(jsonRpcTransport, addressProvider);

    return new SMLAddressProvider(dmlProvider, addressProvider);
  }
