...
function createDAPIAddressProviderFromOptions(options) {
  ...
  if (options.dapiAddressProvider) {
    ...
    if (options.dapiAddressWhiteList) {
      throw new DAPIClientError("Can't use 'dapiAddressWhiteList' with 'dapiAddressProvider' option");
    }
  ...
  if (options.dapiAddresses) {
    ...
    if (options.dapiAddressWhiteList) {
      throw new DAPIClientError("Can't use 'dapiAddressWhiteList' with 'dapiAddresses' option");
    }
  ...
  if (options.seeds) {
    let dapiAddressWhiteList = options.dapiAddressWhiteList || [];
    ...
    if (options.network === 'testnet' && dapiAddressWhiteList.length === 0) {
      dapiAddressWhiteList = networkConfigs.testnet.dapiAddressWhiteList;
    }
    ...
    return new SimplifiedMasternodeListDAPIAddressProvider(
      smlProvider,
      listDAPIAddressProvider,
      dapiAddressWhiteList.map((rawAddress) => new DAPIAddress(rawAddress)),
    );
  }
