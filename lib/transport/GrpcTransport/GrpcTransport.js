...
const TimeoutError = require('../errors/response/TimeoutError');
...
class GrpcTransport {
   ...
   *    DAPIAddressProvider
   * } dapiAddressProvider
   * @param {createTransportError} createTransportError
  ...
  constructor(
    ...
    this.createTransportError = createTransportError;
  ...
  async request(ClientClass, method, requestMessage, options = { }) {
    ...
    try {
      ...
      const responseError = this.createTransportError(error, address);
